...
class ModelCheckpoint(Callback):
    ...
    def on_epoch_end(self):
        ...
        if self.monitor not in self.trainer.logger.metrics:
            print(f"[ModelCheckpoint] Metric {self.monitor} not found in logger. Skipping checkpoint.")
            return
        ...
        if self.previous_best is not None and self.save_best_only:
            if self.direction == 'down':
                if self.previous_best <= trainer_quantity:
                    print(f"[ModelCheckpoint] No improvement. Current: {trainer_quantity} - Previous {self.previous_best}")
                    return
            else:
                if self.previous_best >= trainer_quantity:
                    print(f"[ModelCheckpoint] No improvement. Current: {trainer_quantity} - Previous {self.previous_best}")
