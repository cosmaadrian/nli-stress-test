...
def load_model_by_dir(name):
    checkpoint_path = f'{os.path.abspath(os.path.dirname(__file__))}/{name}/*.model.ckpt'
    print('::: Loading model from:', checkpoint_path)
    checkpoints = glob.glob(checkpoint_path)
    return torch.load(checkpoints[-1])

def load_model_by_name(name, kind = 'best'):
    checkpoint_path = f'{os.path.abspath(os.path.dirname(__file__))}/../checkpoints/{name}/{kind}/*.model.ckpt'
    checkpoints = glob.glob(checkpoint_path)
    print('::: Loading model from:', checkpoint_path, checkpoints)
    return torch.load(checkpoints[-1])
